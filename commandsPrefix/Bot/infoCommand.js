const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ApplicationCommandType } = require("discord.js"); 
...
module.exports = {
    ...
    run: async (client, message, config, db) => {
        ...
        message.reply({ embeds: [embedInfo], components: [selectMenuInfo, BotaoInfo1, BotaoInfo2]}).then(teste => {
            ...
            coletor.on('collect', async (coletado) => {
                ...
                if(valor === "inicio") {
                    teste.edit({ embeds: [ embedInfo ], components: [selectMenuInfo, BotaoInfo1, BotaoInfo2]})
                }
